...
- **Limits**: `intent_judge_timeout_sec` (15s). `num_ctx` = shared `llm_num_ctx` (default 8192; the old per-call 8192-vs-4096 mix against transcript-buffer truncation cost an ~8.5s runner reload per alternation with 4096 callers — raise `llm_num_ctx` instead if long transcripts ever truncate).
