...
- **Inputs**:
  ...
  - **Reply-language clamp** (gating: `cfg.tts_engine in {piper, chatterbox}`, the English-only voices): a final "reply in English, overrides everything above" rule appended LAST in the system message so recency makes it win over earlier in-language pulls (the time/date answer clause, plan steps written in the user's language). Keeps the reply language consistent for the English-only TTS voice path. Removed when a multilingual TTS engine is added. Guarded by `evals/test_reply_language_consistency.py`.
