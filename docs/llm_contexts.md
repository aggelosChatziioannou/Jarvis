...
- **Inputs**:
  ...
  - **Reply-language clamp** (gating: `cfg.tts_engine in {piper, chatterbox}`, the English-only voices): a final "reply in English, overrides everything above" rule appended LAST in the system message so recency makes it win over earlier in-language pulls (the time/date answer clause, plan steps written in the user's language; explicitly covers apologies/error reports/clarifying questions — those turns mirror the user's language hardest). The clamp is PROBABILISTIC (live chat decodes at the model default temperature), so it is backed by the deterministic enforcement pass below. Guarded by `evals/test_reply_language_consistency.py` (incl. the Greek tool-failure apology case). Removed when a multilingual TTS engine is added.
  - **Reply-language enforcement (translation pass, Step 10)**: after the final reply is settled and before TTS/publish, if it still contains non-Latin letters (`_NON_LATIN_LETTERS` — Greek/Cyrillic/CJK) a ONE-shot greedy rewrite runs via `call_llm_direct` (`ollama_chat_model`, temp 0.0, timeout 12s, prompt "rewrite entirely in English, keep meaning/tone/brevity"). Translation has no pull toward the user's language, so it is reliable where regeneration is not. Fires only on violations (rare); fail-open — any error keeps the original reply.
...
- **Limits**: `num_ctx` = **shared `llm_num_ctx`** (default 4096 — every shared-brain call requests the SAME size; a per-call 8192 here forced an ~8.5s Ollama runner reload against the 4096 callers on every voice query). Timeout `llm_chat_timeout_sec` (180s). Output cap `num_predict` from `llm_chat_max_tokens` (default 512, omitted when ≤ 0) and optional decode `temperature` from `llm_chat_temperature` (default `-1.0` = unset/model default). Auto-fallback from native to text tool-calls on HTTP 400 (`ToolsNotSupportedError`), sticky for the session. Risk: `fetch_web_page` truncates at 50,000 chars (~37k tokens) — mitigated for SMALL models by tool-result digest (#5) which compresses the payload before it enters the messages history. LARGE models receive the raw payload and may silently see a truncated context.
...
- **Limits**: `intent_judge_timeout_sec` (15s). `num_ctx` = shared `llm_num_ctx` (default 4096; the old explicit 8192 hedge against transcript-buffer truncation cost an ~8.5s runner reload per alternation with 4096 callers — raise `llm_num_ctx` instead if long transcripts ever truncate).
...
- **Trigger**: voice path, Tier 2 of the listener cascade (after the heuristic Tier-1 classifier escalates). With `use_fused_intent` (default True) this is the **live default**: it collapses intent judge (#2) + tool router (#7) + planner (#12) into **one** structured call (~1-2.3s warm vs ~15-21s for the legacy 3-call path).
...
- **Inputs**: transcript, `in_hot_window`, `language`, `last_tts_text`, `pending_confirmation`. The listener sets `pending_confirmation="forgetMemory"` when a forget deletion is awaiting confirmation (`forget_memory.has_fresh_pending()`); a system-prompt rule then makes the router classify that turn in-language — assent → forgetMemory, refusal → no tool — so a refusal can never reach the deletion tool. See `tools/builtin/forget_memory.spec.md` and `evals/test_fused_forget_confirmation_routing.py`.
- **System prompt**: `_build_system_prompt()` advertises a tool catalogue built **dynamically from the live registry** — real `BUILTIN_TOOLS` names + cached MCP tools via `build_tool_catalogue()` — and is rebuilt when the tool/MCP set changes. (Earlier versions used a hardcoded catalogue of fake dotted names that resolved to no real tool; fixed so the model's returned names match the engine allow-list.) Includes a SCREEN AWARENESS rule and a PENDING CONFIRMATION rule (above).
...
- **Limits**: timeout `min(max(intent_judge_timeout_sec, 15.0), 18.0)` (floor 15s — a cold/contended first call must not degrade into the safe default); `num_ctx` = shared `llm_num_ctx` (default 4096), `num_predict 300`, `keep_alive -1` (shares the one warm brain with chat — a finite value here re-armed an unload timer on every voice query); 2 attempts (GREEDY 0.0 first — routing is classification, temp 0.1 sampled live misses like a Greek weather query judged tools=[]; the 0.1 retry only escapes deterministic parse failures); on total failure a safe default `intent=query` with `degraded=True` keeps the pipeline alive (the listener passes `None` so the engine runs its own router). Measured warm latency on qwen3.5:9b-4k @ RTX 5070 Ti: ~1.0-2.3s (median 1.6s over a 30-case battery) after the `localhost`→`127.0.0.1` base-URL normalisation in `load_settings()` removed a flat ~2s Windows IPv6-fallback penalty from every new Ollama connection.
...
- **Trigger**: top of each agentic-loop iteration when the plan from #12 still has unexecuted tool steps AND either (a) `use_text_tools` is True (SMALL models, every step), or (b) the next plan step names a screen-perception tool (`seeScreen`/`readScreen`/`locateOnScreen`, via `step_names_vision_tool`), or (c) the next plan step names `forgetMemory`, or (d) the next plan step's HEAD token names an allow-listed tool (`step_names_allowed_tool()`) — (b)/(c)/(d) forced on **any** model size because a large model trusted to call an advised tool natively either hallucinates the result instead of looking ("your screen is mostly blank"; "Bitcoin is hovering around 650 dollars" with `getStockPrice` never invoked; a fabricated "I've scanned my inbox"; "Done — reminder set" over an empty reminders table) or narrates a confabulated deletion instead of actually forgetting. Within (d), fully-concrete steps (`step_is_concrete()`) dispatch via the deterministic fast-parse with the router's args verbatim — no resolver LLM; steps with extra/undeclared keys or prose tails go through the resolver, which strips unknown keys before dispatch. For `forgetMemory` the engine executes with the fused router's own arguments (echoed subject and/or confirm flag); the tool deletes only on a genuine assent signal (explicit confirm or a subject matching the pending proposal) and the pending-aware router routes refusals to no tool, so a forced call can never delete on a refusal/misheard turn. Runs instead of the chat model for that turn. **Fast path skips the LLM entirely** when the step is fully concrete (tool name + `key='value'` args, no `<placeholder>`); a bare `forgetMemory` step fast-parses without a resolver LLM call.
...
- **forgetMemory recall** ([src/jarvis/tools/builtin/forget_memory.py](src/jarvis/tools/builtin/forget_memory.py)) — `ollama_embed_model` (nomic-embed-text), NOT a chat call. Only on the **fallback** path: when strict token matching finds no fact to forget, the tool embeds the subject + each candidate fact line and proposes any line with cosine ≥ `memory_forget_semantic_threshold` (default 0.82). Fail-open (no embed server → strict-only). Catches paraphrases ("resides in London" vs stored "lives in London"). Widens only the proposal; deletion still needs explicit confirmation.
...
- **Limits**: `reminder_parse_timeout_sec` (8s), `num_ctx` = shared `llm_num_ctx` (the old private 1024 forced an ~8.5s runner reload that alone blew the 8s timeout — every parse failed), `temperature: 0.0`, no thinking. Fails closed — an unresolved time creates no reminder rather than guessing.
