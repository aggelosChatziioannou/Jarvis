...
- **Trigger**: top of each agentic-loop iteration when the plan from #12 still has unexecuted tool steps AND either (a) `use_text_tools` is True (SMALL models, every step), or (b) the next plan step names a screen-perception tool (`seeScreen`/`readScreen`/`locateOnScreen`, via `step_names_vision_tool`), or (c) the next plan step names `forgetMemory` AND no proposal is pending (the PROPOSE turn) — both (b) and (c) forced on **any** model size because a large model trusted to call them natively either hallucinates the screen instead of looking, or narrates a confabulated deletion instead of actually forgetting. The `forgetMemory` CONFIRM turn is NOT force-executed (it is left to the chat model, which emits `forgetMemory(confirm=true)` on judged assent), because forcing it would mean guessing consent. Runs instead of the chat model for that turn. **Fast path skips the LLM entirely** when the step is fully concrete (tool name + `key='value'` args, no `<placeholder>`); the bare `forgetMemory` step fast-parses to `(forgetMemory, {})` and the tool derives its subject from the user's utterance, so no resolver LLM call fires for it.
