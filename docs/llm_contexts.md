...
- **Limits**: timeout `min(max(intent_judge_timeout_sec, 15.0), 18.0)` (floor 15s — a cold/contended first call must not degrade into the safe default); `num_ctx` = shared `llm_num_ctx` (default 4096), `num_predict 300`, `keep_alive -1` (shares the one warm brain with chat — a finite value here re-armed an unload timer on every voice query); 2 attempts (temp 0.1 then greedy 0.0); on total failure a safe default `intent=query` with `degraded=True` keeps the pipeline alive (the listener passes `None` so the engine runs its own router). Measured warm latency on qwen3.5:9b-4k @ RTX 5070 Ti: ~1.0-2.3s (median 1.6s over a 30-case battery) after the `localhost`→`127.0.0.1` base-URL normalisation in `load_settings()` removed a flat ~2s Windows IPv6-fallback penalty from every new Ollama connection.
