...
- **Trigger**: top of each agentic-loop iteration when the plan from #12 still has unexecuted tool steps AND either (a) `use_text_tools` is True (SMALL models, every step), or (b) the next plan step names a screen-perception tool (`seeScreen`/`readScreen`/`locateOnScreen`, via `step_names_vision_tool`), or (c) the next plan step names `forgetMemory` — both (b) and (c) forced on **any** model size because a large model trusted to call them natively either hallucinates the screen instead of looking, or narrates a confabulated deletion instead of actually forgetting. For `forgetMemory` the engine executes with the fused router's own arguments (echoed subject and/or confirm flag); the tool deletes only on a genuine assent signal (explicit confirm or a subject matching the pending proposal) and the pending-aware router routes refusals to no tool, so a forced call can never delete on a refusal/misheard turn. Runs instead of the chat model for that turn. **Fast path skips the LLM entirely** when the step is fully concrete (tool name + `key='value'` args, no `<placeholder>`); a bare `forgetMemory` step fast-parses without a resolver LLM call.
