...
class TestCpuOptimisations:
    ...
    def test_gpu_does_not_get_cpu_optimisations(self):
        """CUDA mode keeps timestamps enabled. condition_on_previous_text is
        now `False` on every device (see listening.spec.md: command-style
        ASR processes utterances independently so context carry-over only
        propagates errors)."""
