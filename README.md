...
- **Built-in Tools** - Screenshot OCR, web search (DuckDuckGo → Brave → Wikipedia fallback chain with auto-fetch), weather, file access, nutrition tracking, location awareness, reminders, plus a tool-discovery escape hatch the agent uses to widen its own toolset mid-reply
...
- **Reminders** - Set natural-language reminders ("remind me in 20 minutes", "every Monday at 8am", Greek too); they fire with a spoken alert and a system-tray notification when due, all offline
