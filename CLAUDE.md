...
| `src/jarvis/tools/builtin/forget_memory.spec.md` | forgetMemory tool: voice "forget that ...", unbypassable propose→confirm, strict-then-semantic fail-open recall, utterance-derived subject, engine force-exec + pending-as-consent, no_match/removed discriminators | Deletion is irreversible + voice-driven, so propose→confirm is unbypassable; recall only widens the proposal, never the deletion; force-executed like vision; refusals are never routed to a deletion tool |
...
| `src/jarvis/vision/vision.spec.md` | Vision & Screen Interaction Engine: capture, DPI/coord mapping, Tesseract + qwen2.5vl perception, click/type/scroll with safety modes + two-turn confirmation | Tools return raw data; screenshots never hit disk; Tesseract for text + vision model for grounding fallback; no hardcoded language patterns; safety guards unbypassable |
| `src/jarvis/reminders/reminders.spec.md` | Time-triggered spoken + tray reminders: SQLite store, dateparser+LLM parsing, poll-loop firing, delivery, tools | Reminders are NOT memory; no separate scheduler (reuses the daemon poll loop); fail-open; parsing fails closed; core never imports Qt |
| `wakeword-training/studio/studio.spec.md` | Guided wake-word recording studio: resumable session plan, capture-time QC, manifest, trainer export | Plan is the source of truth (deterministic prompt_ids); QC at capture; recordings never committed; traps never contain the wake phrase |
